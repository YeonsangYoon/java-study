 ...
 *          실행 가능한 소스 코딩(정상수행)
 *       } catch {
 *          try에 있는 소스를 실행하는 과정에서 에러가 발생시 처리되는 영역
 *       } finally {
 ...
 *       *** try절에 에러 발생시에는 에러가 난 곳부터는 수행하지 않는다
 ...
 *         => 라이브러리
 *            java.lang / java.util => UnCheckedException : 예외처리를 반드시 하는 것은 아님
 *            java.io / java.net/ java.sql => CheckedException : 예외처리를 반드시 한다
 *         형식)
 *            [접근제어자] 리턴타입 메소드명() throws 예외처리클래스, ...
 *            
 *            => 처리
 *            메소드() throws 예외처리클래스{
 *               메소드호출
